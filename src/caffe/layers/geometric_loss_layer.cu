...
//  N = shape(0)  C = shape(1)  H = shape(2)  W = shape(3)
  ...
  for( size_t i = 0; i < shape[0]; ++i ){
    for( size_t j = 0; j < (shape[1]/num_dim_ - 1); ++j ){
      caffe_gpu_add(
        spatial_dim,
        diff_.gpu_data() + i*image_dim,
        diff_.gpu_data() + i*image_dim + (j+1)*num_dim_*spatial_dim,
        diff_.mutable_gpu_data() + i*image_dim);
    }
  }
  ...
  for( size_t i = 0; i < shape[0]; ++i ){
    for( size_t j = 0; j < (shape[1]/num_dim_ - 1); ++j ){
      caffe_gpu_memcpy(
        spatial_dim,
        diff_.gpu_data() + i*image_dim,
        diff_.mutable_gpu_data() + i*image_dim + (j+1)*num_dim_*spatial_dim);
    }
  }
  Dtype dot;
